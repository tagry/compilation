...
primary_expression
: IDENTIFIER {
	$$.type = hachtab[hachage($1)][etat].type; asprintf(&$$.code, "load %s", $1);
	
	struct symbol_t e = findtab($1);
	
	if(!rechercheTout($1))
	{
		fprintf(stderr, "L'identifiant %s : n'est pas déclaré\n",$1); 
	}
	else
	{
...
| INC_OP unary_expression {$$.type = $2.type;
	 asprintf(&$$.var, "%%x%d", tmp_var_name());
	 if($2.type == T_FLOAT)
		 asprintf(&$$.code, "%s = fadd f32 %s, 1\n store f32 %%x%d, f32 %s", $$.var, $2.code, step, $$.var);
	 else
...
| unary_operator unary_expression {$$.type = $2.type;
	 asprintf(&$$.var, "%%x%d", tmp_var_name());
	 if($2.type == T_FLOAT)
		 asprintf(&$$.code, "%s = fsub f32 0, %s\n store f32 %%x%d, f32 %s", $$.var, $1.code, step, $$.var);
	 else
...
: unary_expression {$$.type = $1.type;// on vire tout
	 asprintf(&$$.var, "%%x%d", tmp_var_name());
	 if($1.type == T_FLOAT)
		 asprintf(&$$.code, "store f32 %s, f32 %s\n", $1.code, $$.var);
	 else
		 asprintf(&$$.code, "store i32 %s, i32 %s\n", $1.code, $$.var);
 }
| multiplicative_expression '*' unary_expression    {if($3.type == T_FLOAT || $1.type == T_FLOAT)
	 ...
	 asprintf(&$$.var, "%%x%d", tmp_var_name());
	 if($3.type == T_FLOAT)
		 asprintf(&$$.code, "%s = fmul f32 %s, %s\n store f32 %%x%d, f32 %s", $$.var, $1.code, $3.code, step, $$.var);
	 else
		 asprintf(&$$.code, "%s = mul i32 %s, %s\n store i32 %%x%d, i32 %s\n", $$.var, $1.code, $3.code, step, $$.var);
  }
...
assignment_operator
: '=' {asprintf(&$$.code, "=");}
| MUL_ASSIGN {asprintf(&$$.code, "*=");}
| ADD_ASSIGN {asprintf(&$$.code, "+=");}
| SUB_ASSIGN {asprintf(&$$.code, "-=");}
;
...
compound_statement {
    ...
    if(!strcmp($1.code, "INT"))
        asprintf(&$$.code, "define i32 %s { %s }", $2.code, $4.code);
    else if(!strcmp($1.code, "FLOAT"))
        asprintf(&$$.code, "define float %s { %s }", $2.code, $4.code);} 
;
